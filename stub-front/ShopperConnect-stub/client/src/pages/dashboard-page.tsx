...
export default function DashboardPage() {
  ...
  const { data: spendingCategories, isLoading: isSpendingCategoriesLoading } = useQuery({
    queryKey: ["/api/spending-categories"],
  });
  
  const isLoading = isBudgetSummaryLoading || isTransactionsLoading || isSpendingCategoriesLoading;
  
  const initials = user ? (user.firstName?.charAt(0) || '') + (user.lastName?.charAt(0) || '') : user?.username?.substring(0, 2) || 'ИП';
  
  // Проверяем, нужно ли показать диалог планирования бюджета
  useEffect(() => {
    if (budgetSummary?.needsBudgetPlanning && user?.role === 'PARENT') {
